define(["require", "exports", './core/memory', './context', './core/controller', './format/format', './format/cso', './format/pbp', './format/iso'], function(require, exports, Memory, EmulatorContext, controller, format, Cso, Pbp, _Iso) {
    var Iso = _Iso.Iso;

    var PspCtrlButtons = controller.PspCtrlButtons;
    ...
    var Emulator = (function () {
        ...
        Emulator.prototype._loadAsync = function (asyncStream, pathToFile) {
            ...
            return format.detectFormatAsync(asyncStream).then(function (fileFormat) {
                ...
                switch (fileFormat) {
                    case 'ciso':
                        return Cso.fromStreamAsync(asyncStream).then(function (asyncStream2) {
                    ...
                    case 'iso':
                        return Iso.fromStreamAsync(asyncStream).then(function (iso) {
                            var isoFs = new hle.vfs.IsoVfs(iso);
                            _this.fileManager.mount('umd0', isoFs);
                            _this.fileManager.mount('disc0', isoFs);

                            return isoFs.open('PSP_GAME/SYSDIR/BOOT.BIN', hle.vfs.FileOpenFlags.Read, parseInt('777', 8)).readAllAsync().then(function (data) {
