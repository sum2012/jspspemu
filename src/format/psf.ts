...
class HeaderStruct {
	...
	static struct = struct.StructClass.create<HeaderStruct>(HeaderStruct, [
...
class EntryStruct implements IEntryStruct {
	...
	static struct = struct.StructClass.create<EntryStruct>(EntryStruct, [
...
export class Psf {
	...
	load(stream: Stream) {
		...
		var entries = struct.StructArray.create<EntryStruct>(EntryStruct.struct, header.numberOfPairs).read(stream);
