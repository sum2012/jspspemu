...
export class ThreadManForUser {
	...
	sceKernelExitDeleteThread = createNativeFunction(0x809CE29B, 150, 'uint', 'HleThread/int', this, (currentThread: Thread, exitStatus: number) => {
        currentThread.exitStatus = exitStatus;
        currentThread.stop();
        throw (new CpuBreakException());
    });
...
class SceKernelSemaInfo {
	...
	static struct = struct.StructClass.create<SceKernelSemaInfo>(SceKernelSemaInfo, [
...
class SceKernelThreadInfo {
	...
	static struct = struct.StructClass.create<SceKernelThreadInfo>(SceKernelThreadInfo, [
		{ type: struct.Int32, name: 'size' },
		{ type: struct.Stringz(32), name: 'name' },
